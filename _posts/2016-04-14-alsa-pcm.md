...
PCM设备状态自动进入RUNNING的事件(默认值为 1 frame).

1. __playback__

  前提：设备处于 **PREPARED** 状态, start threshold < buffer size(i.e. start threshold enabled):

  * write trigger: 写入后，如果ring buffer 中的数据 >= start\_threshold, PCM设备进入 **RUNNING**
  * 手动 start (晦涩的部分 - -):
...
2. __capture__ 
  ...
  * read trigger: 如果ALSA应用要求读的数据 >= start\_threshold, PCM设备会进入 **RUNNING** 状态
