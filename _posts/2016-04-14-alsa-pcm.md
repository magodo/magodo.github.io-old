...
声卡设备在硬件层面有一个ring buffer，同时在alsa lib中也维护一个ring buffer于内存当中：
...
1. `snd_pcm_avail_update()`: 获得当前ready的frame数(读/写)，并且为application层的数据传输做好准备
...
2. -ESTRPIPE
  表示设备处于 _suspended_. 这时候需要循环调用`snd_pcm_resume()` 直到它返回-EAGAIN( _setup_ )或者0 ( _prepare_ ).
...
ALSA PCM设备对于硬件参数，使用了一种"逐步限制"的机制(refinement system) - `snd_pcm_hw_params_t`. 应用程序在一开始设定所有支出的HW参数范围，然后通过设置其中的某几个参数为定值，直到其他参数都被限定为止。
