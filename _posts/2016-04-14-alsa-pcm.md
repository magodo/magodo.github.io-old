...
* 在playback的时候，ALSA应用通过DMA将buffer中的数据传给PCM设备的buffer. 当ALSA设备进入RUNNING状态，每当kernel维护的ring buffer中的数据到达一个period之后，会向kernel发送硬件中断，于是kernel在handler中会将数据由自身维护的ring buffer传输至硬件声卡的buffer;
* 在capture的时候，PCM设备通过DMA将buffer中的数据传给ALSA应用的buffer. 当ALSA设备进入RUNNING状态，声卡设备会不断地采集到数据，每采集一个period的数据后，声卡设备会发送硬件中断到kernel，kernel在handler中将数据由硬件buffer传输至自己维护的ring buffer中.
...
对于某个HW参数，会有以下几种形式的API可供使用：
    ...
    snd_pcm_hw_params_set_xxx: 设置某个硬件参数，如果当前硬件不支持该值，则设置的实际值会与期望值不同。dir(in),用于指定当期望值不被支持的时候，硬件会选用比该值小的实际值还是大的实际值。(期望值 --(dir)--> 实际值)
    snd_pcm_hw_params_set_xxx_near: 设置某个硬件参数，如果当前硬件不支持该值，则设置的实际值会与期望值不同。dir(in),用于指定当期望值不被支持的时候，会选用比该值小的实际值还是大的实际值。同时，实际被设置的值也会被转换为大约值传出来，这个大约值可能由于类型关系而于实际值有所区别(例如rate的实际值是个浮点数，而大约值却是个ulong)。(期望值 --(dir)--> 实际值 --(round)--> 大约值)
    ...
    snd_pcm_hw_params_test_xxx: 测试在当前参数空间内，某个硬件参数是否可以被设置
